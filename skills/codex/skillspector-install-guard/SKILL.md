...
description: Scan third-party agent extensions with SkillSpector before installation. Use when Codex is asked to install, approve, recommend, update, or review a skill, MCP server, plugin, connector, slash-command pack, agent repo, zip archive, local SKILL.md bundle, or similar agent capability from a directory, file URL, Git URL, or archive.
---
...
- Prefer the scanner's JSON output for agent reasoning, then summarize it for the user in plain language.

## Commands

```bash
bash skills/codex/skillspector-install-guard/scripts/scan-before-install.sh ./path/to/skill
bash skills/codex/skillspector-install-guard/scripts/scan-before-install.sh https://github.com/org/repo
bash skills/codex/skillspector-install-guard/scripts/scan-before-install.sh ./skill.zip --llm
```

## Notes

- Use this skill for skills, MCP servers, plugin bundles, connector repos, and installable agent extensions.
- If the user only asks for a conceptual explanation and no install is happening, summarizing likely risks can be enough. The full scan becomes mandatory before a real install or approval.
- When another project file already contains install-policy instructions, follow the stricter rule.
